...
Q.state.reset({ score: 0, strength: 100 });
Q.SPRITE_PLAYER = 1;
...
Q.Sprite.extend("Player",{
  ...
  enemyHit: function(data) {
	...
	Q.state.dec("strength", 25);
   // this.p.strength -= 25;
   // Q.stageScene('hud', 3, this.p);
    if (Q.state.get("strength") == 0) {
      this.resetLevel();
    }
  },
...
Q.scene('hud',function(stage) {
  ...
  var label = container.insert(new Q.UI.Text({x:200, y: 20,
    label: "Score: " + Q.state.get("score"), color: "white",weight: "normal",
        size:18 }));

   var label1 = container.insert(new Q.UI.Text({x:50, y: 20,
    label: "Health: " + Q.state.get("strength"), color: "white",weight: "normal",
        size:18 }));
  container.fit(20);
...
label.p.label = "Score: " + Q.state.get("score");
  ...
  Q.state.on("change.strength", function () {
label1.p.label = "Health: " + Q.state.get("strength");
});
});
